...
function categoryBadge(cat: string | null | undefined) {
  ...
  if (cat === "Warm") return <Badge className="gap-1 text-xs bg-amber-500 hover:bg-amber-500"><Thermometer className="size-3" />Warm</Badge>;
...
// Merge server scores into lead list (used for immediate UI update before refresh)
...
function buildCounts(leads: LeadWithScore[]): ScoreCounts {
  ...
  for (const l of leads) {
    if (!l.score) c.Unscored++;
    else c[l.score.lead_category]++;
  }
  return c;
}
...
function Dashboard() {
  ...
  const handleScoreAll = async () => {
    ...
    try {
      ...
      // Apply scores immediately to current lead list (no DB read needed)
      setLeads((prev) => applyScores(prev, result.scores));
      setTotalContacts((t) => t); // trigger re-render
      toast.success(`Scored ${result.scores.length} leads in ${result.elapsed}ms`);
      // Also refresh from DB to pick up any persisted data
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={handleScoreAll}
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatCard icon={<Flame       className="size-4 text-red-500"   />}        label="Hot leads"  value={scoreCounts.Hot}  pct={totalContacts ? Math.round(scoreCounts.Hot  / totalContacts * 100) : 0} />
          <StatCard icon={<Thermometer className="size-4 text-amber-500" />}        label="Warm leads" value={scoreCounts.Warm} pct={totalContacts ? Math.round(scoreCounts.Warm / totalContacts * 100) : 0} />
          <StatCard icon={<Snowflake   className="size-4 text-sky-500"   />}        label="Cold leads" value={scoreCounts.Cold} pct={totalContacts ? Math.round(scoreCounts.Cold / totalContacts * 100) : 0} />
        </section>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-red-500   inline-block" />Hot ({scoreCounts.Hot})</span>
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-amber-400 inline-block" />Warm ({scoreCounts.Warm})</span>
                <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-sky-400   inline-block" />Cold ({scoreCounts.Cold})</span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Upload CSV or XLSX. Contacts are validated, deduplicated, and auto-scored by AI.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>AI-classified contacts. Click <RotateCcw className="size-3 inline" /> to re-run the model on any row.</CardDescription>
              </div>
                        ...
                        <TableBody>
                          {filteredLeads.map((lead) => (
                            <TableRow key={lead.id}>
                              <TableCell className="font-mono text-xs max-w-[200px] truncate">{lead.email}</TableCell>
                              ...
                              <TableCell>
                                {lead.score
                                  ? <ScoreBar score={lead.score.ai_score} />
